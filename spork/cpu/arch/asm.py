    ...
    def __str__(self):
        if self.lines:
            return self.args[0].format(
                *self.args[1:],
                **{loc: f"line {indexes[0] + 1}" for loc, indexes in self.locs.items()},
            )
        else:
            return self.args[0].format(
                *self.args[1:],
        ...
        if m["instr"]:
            try:
                ...
                raise TranslationError(
                    f"{{0}} at {{loc}}", error, loc=(index,), lines=True
                ) from None
        if m["direct"]:
            if m["direct"] == ".word":
                ...
                raise TranslationError(
                    f"Unknown directive {m['direct']} at {{loc}}",
                    loc=(index,),
                    lines=True,
                )
    ...
    for index, elem in enumerate(input):
        if isinstance(elem, mc.Label):
            ...
            try:
                ...
                elem.encode(words)
                encoding = " ".join(
                    "{:0{}X}".format(word, len(elem.coding) // 4) for word in words
                )
            ...
            raise TranslationError(
                f"Unrecognized value {repr(elem)} of type {elem_type_name} "
                f"at {{loc}}",
                loc=(index,),
            )
    output.append("")
...
# TODO convert to class that captures the pointers and label positions


class Assembler:
    "Boneless Assembler Wrapped in an class"
    ...
    def assemble(input, *, instr_cls):
        ...
        def translate(elem, output, n_pass, *, indexes=(), allow_unresolved=None):
            ...
            if elem is None and n_pass == 1:
                ...
                if n_pass == 1:
                    if elem.name in label_addrs:
                        raise TranslationError(
                            f"Label {repr(elem.name)} at {{new}} has the same name "
                            f"as the label at {{old}}",
                            new=indexes,
                            old=label_locs[elem.name],
                        )
                ...
                try:
                    ...
                    try:
                        ...
                        length = elem(
                            lambda sym: resolve(elem_addr + length, sym)
                        ).encode(output)
                        ...
                        if allow_unresolved is None:
                            ...
                            rel_length = elem(lambda sym: 0).encode(
                                output, use_longest=True
                            )
                            assert (
                                length == rel_length
                            ), f"Illegal longest encoding at {indexes}"
                        else:
                            # ... raise an error otherwise.
                            raise TranslationError(
                                f"{{0}} at {{loc}}", error, loc=indexes
                            ) from None
            elif hasattr(elem, "__call__"):
                translate(
                    elem(lambda sym: resolve(elem_addr, sym)),
                    output,
                    n_pass,
                    indexes=indexes,
                    allow_unresolved=allow_unresolved,
            ...
            if length is not None:
                ...
                assert (
                    length <= old_length
                ), f"Expansion at {indexes}: {old_length} to {length}"
